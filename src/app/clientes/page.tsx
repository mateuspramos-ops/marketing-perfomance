...
export default function ClientsPage() {
  ...
  const totals = useMemo(
    () => ({
      projetos: filtered.reduce((acc, d) => acc + d.projetos, 0),
      atividades: filtered.reduce((acc, d) => acc + d.atividades, 0),
      etapas: filtered.reduce((acc, d) => acc + d.etapas, 0),
  ...
  const porCliente = useMemo(() => {
    const nomes = [...new Set(filtered.map((d) => d.cliente))];
    return nomes
      .map((cliente) => {
        const linhas = filtered.filter((d) => d.cliente === cliente);
        return {
          cliente,
          projetos: linhas.reduce((acc, d) => acc + d.projetos, 0),
          atividades: linhas.reduce((acc, d) => acc + d.atividades, 0),
          etapas: linhas.reduce((acc, d) => acc + d.etapas, 0),
      ...
      return (
        <div className="glass-panel rounded-xl border-0 p-3 text-sm">
          ...
          <p className="mt-1 text-muted-foreground">
            {formatHoras(payload[0].value)} dedicadas
          </p>
        </div>
      );
    }
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.div variants={itemVariants} className="mb-8">
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">
              Projetos por <span className="text-gradient-brand">Cliente</span>
            </h1>
            <p className="text-muted-foreground mt-1.5">
              Volume de trabalho e horas dedicadas a cada cliente
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants}>
            <KpiGrid>
              <KpiCard
                ...
                value={formatNumber(porCliente.length)}
                subtitle="no período"
              ...
              <KpiCard
                ...
                subtitle={`${formatNumber(totals.etapas)} etapas`}
          ...
          <motion.div variants={itemVariants} className="grid gap-6 md:grid-cols-2 mt-6">
            ...
            <Card className="glass-panel border-0 transition-all duration-300 hover:shadow-[0_25px_55px_-25px_var(--primary)]">
              ...
              <CardContent>
                <div className="h-[340px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={porCliente} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
